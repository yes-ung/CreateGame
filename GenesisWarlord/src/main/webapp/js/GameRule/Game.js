...
// 선택 표시 및 정보창을 갱신합니다.
    ...
    $slider.slick({
        ...
        slidesToScroll: 1,         // 기본 이동은 1개
        swipeToSlide: true,        // ✅ 강하게 넘기면 여러 개 넘어감
        infinite: false
    });
    // 원하는 슬라이드 번호로 넘어가는 검색창
    $('#goToSlideBtn').off('click').on('click', function () {
        const totalSlides = $('.StageButtonSlider .slick-slide').length;
        const visibleSlides = 3; // slidesToShow 값과 동일

        let slideIndex = parseInt($('#slideInput').val(), 10) -1;

        if (!isNaN(slideIndex)) {
            // 최대 이동 가능 인덱스 보정 (전체 - 보여줄 개수)
            const maxIndex = totalSlides - visibleSlides;
            if (slideIndex > maxIndex) slideIndex = maxIndex;
            if (slideIndex < 0) slideIndex = 0;

            $('.StageButtonSlider').slick('slickGoTo', slideIndex);
        }
    });
},
